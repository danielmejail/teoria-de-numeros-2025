...
% \begin{ejemEuclidianos}\label{ejem:euclidianos:polinomios}
	...
	% $\polinomios\Racionales$, $\polinomios\Reales$ y
	% $\polinomios\Complejos$ son anillos conmutativos con la suma
	% y el producto usuales. M\'as en general, si $k$ es un cuerpo,
	% $\polinomios k$ es un anillo conmutativo. M\'as en general aun,
	% si $A$ es un anillo conmutativo, $\polinomios A$ es un anillo
	% conmutativo. Un caso particular de esto \'ultimo es
	% $\polinomios\Enteros$. Los polinomios en varias indeterminadas
	% tambi\'en son anillos:
	% $\polinomios[X,Y]\Enteros=\polinomios[Y]{\polinomios[X]\Enteros}$
	% es un anillo conmutativo.%
	% \footnote{
		% T\'ecnicamente, estos $\polinomios[X,Y]\Enteros$ y
		% $\polinomios[Y]{\polinomios[X]\Enteros}$ no son lo mismo,
...
\input{./tex/herramientas/ejercicios-estructuras.tex}
